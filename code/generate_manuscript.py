...
# =================== FIGURE CAPTIONS (separate list) ===================
sec('图表图注', level=1)

captions = [
    '图1  研究筛选流程图。DHS: 人口与健康调查项目；GHO: 全球卫生观察站。',
    '图2  烟草使用财富相关梯度。（A）各国吸烟率跨财富五分位轨迹（灰色细线）及均值梯度'
    '（蓝色圆点，误差线示±1 SD）。（B）全烟草使用均值梯度（绿色方块及误差线）。'
    '（C）各国吸烟SII分布直方图。虚线示CI = 0；实线示均值SII。Q1–Q5：财富五分位（最贫困至最富裕）。',
    '图3  当前吸烟财富相关集中指数（CI）森林图。圆点为CI点估计值；水平线示95%置信区间。'
    '垂直虚线示CI = 0；红色实线示全样本均值CI。CI < 0表示吸烟集中于较贫困人群。',
    '图4  吸烟集中指数（CI）跨三个社会经济维度的比较。（A）财富五分位维度。（B）教育水平维度。'
    '（C）城乡居住地维度。每个圆点代表一个国家-调查观测；红色垂直线示各维度内均值CI。',
    '图5  全烟草使用与纯吸烟财富相关CI的配对比较。（A）CI散点图：横轴为纯吸烟CI，'
    '纵轴为全烟草使用CI。橙色点示CI_all_tobacco < CI_smoked；灰色点示方向相反。'
    '对角虚线示y = x。（B）配对哑铃图：线段连接同一国家内全烟草使用（绿色）与纯吸烟'
...
for cap in captions:
    cpp = doc.add_paragraph(cap)
    cpp.paragraph_format.first_line_indent = Cm(0)
    cpp.paragraph_format.space_after = Pt(4)
    for run in cpp.runs:
        run.font.size = Pt(10)
